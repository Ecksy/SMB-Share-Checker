...
clientname=$1
FILE=$2
OUT_FILE=$clientname-smb_check_no_auth-serial.txt
AUTH_FILE=
#Auth file format
#username = <USER>
#password = <PASSWORD>
...
while read p; do
	...
	if [[ $RESULT == *"NT_STATUS_ACCESS_DENIED"* ]]; then
		...
		while read -r line; do
			...
			echo "$COMMAND" >> $clientname-smbclient_responses-serial.txt
			echo "$SHARE_RESULT" >> $clientname-smbclient_responses-serial.txt
